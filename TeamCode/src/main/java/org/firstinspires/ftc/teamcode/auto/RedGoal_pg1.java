...
@Disabled
@Configurable
@Autonomous(name = "RedGoal_pg1", group = "Examples")
public class RedGoal_pg1 extends OpMode {
    ...
    private final Pose scorePosePreload = new Pose(100, 96, Math.toRadians(220)); // Scoring Pose of our robot for the preload. It is facing the goal at a -45 degree angle.
    private final Pose pickup1Setup = new Pose(93, 81, Math.toRadians(0)); // Setup to pickup the highest set of balls
    private final Pose pickup1Pose = new Pose(114, 81, Math.toRadians(0));// Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose gateSetup = new Pose(118, 75, Math.toRadians(0)); // Stand infront of the gate
    private final Pose gateOpen = new Pose(128, 75, Math.toRadians(0)); // Open the gate
    private final Pose scorePose1 = new Pose(100, 96, Math.toRadians(220)); // Scoring Pose of our robot for the first pickup. It is facing the goal at a -45 degree angle.
    ...
    private final Pose leaveTrianglePose = new Pose(127,95, Math.toRadians(180)); // Pose for leaving to triangle
    ...
    public void buildPaths() {
        ...
        leave = follower.pathBuilder()
                .addPath(new BezierLine(scorePose1, leaveTrianglePose))
                .setLinearHeadingInterpolation(scorePose1.getHeading(), leaveTrianglePose.getHeading())
                .build();
    }
